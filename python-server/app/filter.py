...
class WorkerThread(threading.Thread):
    def __init__(self, task_queue, result_queue, rank=0):
        super().__init__()
...
def main():
    ...
    task_queue = [("image1.jpg", "edge_detection"), ("image1.jpg", "color_inversion"),
                  ("image1.jpg", "increase_brightness"), ("image2.jpg", "edge_detection"),
                  ("image2.jpg", "color_inversion"),
                  ("image2.jpg", "increase_brightness")]  # Example task queue
    ...
    if size == 1:
        ...
        threads = []
        for _ in range(local_num_threads):
            thread = WorkerThread(local_task_queue, local_result_queue)
        ...
        if rank == 0:
            ...
            # Process the received results (e.g., save them)
            ...
            threads = []
            for _ in range(num_threads):
                thread = WorkerThread(task_queue, result_queue, rank)
                threads.append(thread)
                thread.start()
